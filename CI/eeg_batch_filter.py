...
for folder_name in remain_list:
    ...
    unified_dataset.eeg_batch.filter(output_path=CI_output_path)
    print(f"Test with default parameters (bandpass) for {folder_name} completed.")

    # Low-pass filter
    unified_dataset.eeg_batch.filter(output_path=CI_output_path, filter_type='lowpass', h_freq=40.0)
    print(f"Test with low-pass filter (h_freq=40.0) for {folder_name} completed.")

    # High-pass filter
    unified_dataset.eeg_batch.filter(output_path=CI_output_path, filter_type='highpass', l_freq=1.0)
    print(f"Test with high-pass filter (l_freq=1.0) for {folder_name} completed.")

    # Bandpass filter with specific frequency range
    unified_dataset.eeg_batch.filter(output_path=CI_output_path, filter_type='bandpass', l_freq=1.0, h_freq=40.0)
    print(f"Test with bandpass filter (l_freq=1.0, h_freq=40.0) for {folder_name} completed.")

    # Notch filter
    unified_dataset.eeg_batch.filter(output_path=CI_output_path, filter_type='notch', notch_freq=50.0)
    ...
    unified_dataset.eeg_batch.filter(output_path=CI_output_path, filter_type='bandpass', l_freq=0.1, h_freq=256.0, auto_adjust_h_freq=True)
    print(f"Test with bandpass filter and auto-adjust h_freq for {folder_name} completed.")

    # Skipping bad data (miss_bad_data=True)
    unified_dataset.eeg_batch.filter(output_path=CI_output_path, miss_bad_data=True)
