...
def con_udatasets(datasets):
    ...
    the 'UnifiedDataset' class. It then calls the `get_locator()` method
    of each dataset, concatenates them, and sets the new locator in a
    copied version of the first dataset using `set_locator()`.

    Parameters
    ----------
    datasets : list of UnifiedDataset
    ...
    """
    if not all(isinstance(ds, UnifiedDataset) for ds in datasets):
